...
var PlaneGon = /*#__PURE__*/function (_React$Component) {
  _inheritsLoose(PlaneGon, _React$Component);

  // load
  function PlaneGon(props) {
    var _this2;
  ...
  _proto.creat = function creat() {
    ...
    var _this$props5 = this.props,
        planeDataPolyGon = _this$props5.planeDataPolyGon,
    ...
    if (planeDataPolyGon) {
      var _loop = function _loop(_i) {
        ...
        if (item.data && itemshow) {
          ...
          var newdom = new BMapGL.Polygon(newdata, {
            domid: newsdomid,
            domtype: 'planeDataPolyGonRoot',
            //自定义类型
            domindex: itemindex,
      ...
      for (var _i = 0; _i < planeDataPolyGon.length || 0; _i++) {
        _loop(_i);
      }
    }
  } // 添加
  ...
  _proto.creatAdd = function creatAdd() {
    ...
    var _this$props6 = this.props,
        planeDataPolyGon = _this$props6.planeDataPolyGon,
        map = _this$props6.map,
        isEnableDragging = _this$props6.isEnableDragging; //传过来的数据

    var newplaneDataPolyGon = planeDataPolyGon ? planeDataPolyGon[planeDataPolyGon.length - 1] : '';

    if (newplaneDataPolyGon) {
      var item = newplaneDataPolyGon;
      var itemindex = planeDataPolyGon.length - 1;
      var itemshow = item.show ? item.show : true;
...
var mapDispatchToProps = function mapDispatchToProps(dispatch) {
  return {
    setdata: function setdata(_this, data) {
      dispatch(_actions.default.removeItem('planeDataPolyGon_BD', data.itemindex));
    }
  };
};

var _default = (0, _reactRedux.connect)(mapStateToProps, mapDispatchToProps)(PlaneGon);
