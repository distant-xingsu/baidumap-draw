...
var StandPoylon = /*#__PURE__*/function (_React$Component) {
  _inheritsLoose(StandPoylon, _React$Component);

  // load
  function StandPoylon(props) {
    var _this2;
  ...
  var _proto = StandPoylon.prototype;
  ...
  _proto.creat = function creat() {
    ...
    var _this$props5 = this.props,
        standDataPoylon = _this$props5.standDataPoylon,
    ...
    if (standDataPoylon) {
      var _loop = function _loop(_i) {
        ...
        if (item.data && itemshow) {
          ...
          var newtopFillColor = item.style ? item.style.topFillColor ? (0, _colorType.hexify)(item.style.topFillColor) : "#00c5ff" : "#00c5ff"; //顶面的颜色，同CSS颜色

          var newtopFillOpacity = item.style ? item.style.topFillColor ? (0, _colorType.getRgbaAlp)(item.style.topFillColor) : 1.0 : 1.0; //顶面透明度，范围0-1

          var newsideFillColor = item.style ? item.style.sideFillColor ? (0, _colorType.hexify)(item.style.sideFillColor) : '#195266' : '#195266'; //测面填充颜色，同CSS颜色  "#195266"

          var newsideFillOpacity = item.style ? item.style.sideFillColor ? (0, _colorType.getRgbaAlp)(item.style.sideFillColor) : 1.0 : 1.0; // 测面填充的透明度，范围0-1
          ...
          var newdom = new BMapGL.Prism(newdata, item.height ? item.height : 20, {
            domid: newsdomid,
            domtype: 'standDataPoylonRoot',
            //自定义类型
            domindex: itemindex,
      ...
      for (var _i = 0; _i < standDataPoylon.length || 0; _i++) {
        _loop(_i);
      }
    }
  } // 添加
  ...
  _proto.creatAdd = function creatAdd() {
    ...
    var _this$props6 = this.props,
        standDataPoylon = _this$props6.standDataPoylon,
        map = _this$props6.map,
        isEnableDragging = _this$props6.isEnableDragging; //传过来的数据

    var newstandDataPoylon = standDataPoylon ? standDataPoylon[standDataPoylon.length - 1] : '';

    if (newstandDataPoylon) {
      var item = newstandDataPoylon;
      var itemindex = standDataPoylon.length - 1;
      var itemshow = item.show ? item.show : true;
...
var mapDispatchToProps = function mapDispatchToProps(dispatch) {
  return {
    setdata: function setdata(_this, data) {
      dispatch(_actions.default.removeItem('standDataPoylon_BD', data.itemindex));
    }
  };
};

var _default = (0, _reactRedux.connect)(mapStateToProps, mapDispatchToProps)(StandPoylon);
